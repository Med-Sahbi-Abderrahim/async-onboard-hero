...
import { formatProgressRatio, hasProgress } from "@/lib/progress-display";
...
export function ClientProgressCard({ clientId, organizationId }: ClientProgressCardProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-2xl font-bold ${getStatusColor(progress.overall)}`}>{progress.overall}%</span>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-muted-foreground">
                  {formatProgressRatio(progress.tasks.completed, progress.tasks.total, "No tasks yet")}
                </span>
              </div>
              {hasProgress(progress.tasks.total) && <Progress value={progress.tasks.percentage} className="h-2" />}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-muted-foreground">
                  {formatProgressRatio(progress.forms.completed, progress.forms.total, "No forms yet")}
                </span>
              </div>
              {hasProgress(progress.forms.total) && <Progress value={progress.forms.percentage} className="h-2" />}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-muted-foreground">
                  {progress.files.uploaded > 0 ? `${progress.files.uploaded} uploaded` : "No files yet"}
                </span>
              </div>
              {progress.files.uploaded > 0 && <Progress value={progress.files.percentage} className="h-2" />}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-muted-foreground">
                  {formatProgressRatio(progress.contracts.signed, progress.contracts.total, "No contracts yet")}
                </span>
              </div>
              {hasProgress(progress.contracts.total) && (
      ...
      {/* Milestone Timeline */}
      <MilestoneTimeline clientId={clientId} organizationId={organizationId} progress={progress} />
    </div>
  );
}
