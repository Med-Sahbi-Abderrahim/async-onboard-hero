    ...
    try {
      ...
      if (inviteCode && data.user) {
        const {
          data: inviteResult,
          error: inviteError
        } = await supabase.rpc("use_early_access_invite", {
      ...
      if (data.user && !data.session) {
        ...
        const {
          data: memberships
        } = await supabase.from('organization_members').select('organization_id').eq('user_id', data.session.user.id).is('deleted_at', null);
        if (memberships && memberships.length > 0) {
          toast({
          ...
          const {
            data: clientData
          } = await supabase.from('clients').select('organization_id').eq('user_id', data.session.user.id).is('deleted_at', null).limit(1).maybeSingle();
          if (clientData) {
            toast({
              title: "Account created!",
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              field
                    ...
                    {passwordStrength.strength && <p className={`text-sm ${passwordStrength.color}`}>Strength: {passwordStrength.strength}</p>}
                    <FormMessage />
                  </FormItem>} />
