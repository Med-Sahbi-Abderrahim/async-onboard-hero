...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (orgId) {
      ...
      const fetchOrgData = async () => {
        const {
          data
        } = await supabase.from('organizations').select('name, logo_url, brand_color').eq('id', orgId).single();
        if (data) {
          setOrgData(data);
        }
      };
    ...
    const checkUser = async () => {
      ...
      if (session) {
        ...
        const {
          data: memberships
        } = await supabase.from('organization_members').select('organization_id').eq('user_id', session.user.id).is('deleted_at', null);
        const {
          data: clientData
        } = await supabase.from('clients').select('organization_id').eq('user_id', session.user.id).is('deleted_at', null);
  ...
  const onSubmit = async (values: LoginFormValues) => {
    ...
    try {
      ...
      const {
        data: memberships
      } = await supabase.from('organization_members').select('organization_id').eq('user_id', data.user.id).is('deleted_at', null);
      const {
        data: clientData
      } = await supabase.from('clients').select('organization_id').eq('user_id', data.user.id).is('deleted_at', null);
      const hasOrgMemberships = memberships && memberships.length > 0;
      const hasClientRecords = clientData && clientData.length > 0;
      ...
      <div className={`hidden lg:flex lg:w-1/2 ${isClientLogin ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : 'gradient-auth'} items-center justify-center p-12`}>
        <div className="max-w-md text-white">
          {isClientLogin ? <>
              ...
              {orgData && <div className="mt-8 p-4 bg-white/10 rounded-lg backdrop-blur-sm">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <div className="flex justify-center mb-6">
            {orgData?.logo_url ? <img src={orgData.logo_url} alt={orgData.name} className="h-16 object-contain" /> : <div className="flex items-center gap-2">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">
              {isClientLogin && orgData ? `Access your ${orgData.name} client portal` : isAgencyLogin && orgData ? `Continue to ${orgData.name} workspace` : 'Enter your credentials to continue'}
            </p>
          </div>
          ...
          {isClientLogin ? <p className="mt-6 text-center text-sm text-muted-foreground">
              First time here? Check your email for your invitation link with access instructions.
            </p> : <p className="mt-6 text-center text-sm text-muted-foreground">
